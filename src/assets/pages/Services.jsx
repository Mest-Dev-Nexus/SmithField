...
const Services = () => {
  ...
  const serviceDetails = [
    ...
    },
    {
      ...
      description: "Our state-of-the-art cold storage facilities preserve freshness and extend shelf life of produce. Our solutions reduce post-harvest losses by up to 40% while maintaining nutritional quality.",
      features: [
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Comprehensive agricultural solutions designed to maximize productivity, 
            sustainability, and profitability for farmers and agribusinesses
          </p>
        </div>
            ...
            <div className="bg-white rounded-xl shadow-lg overflow-hidden mb-8 transition-all hover:shadow-xl border border-gray-100">
              <div className="flex flex-col md:flex-row">
                ...
                <div className="md:w-1/2 p-8 md:p-12 flex flex-col justify-center">
                  <div className="flex items-center mb-6">
                    <div className={`bg-gradient-to-r from-${service.color}-100 to-blue-100 w-16 h-16 rounded-full flex items-center justify-center transform transition-all hover:rotate-12`}>
                      <service.icon className={`text-${service.color}-700`} size={32} />
                    </div>
                  ...
                  <ul className="mb-8 space-y-3">
                    {service.features.map((feature, idx) => (
                      <li key={idx} className="flex items-center text-gray-700">
                        <span className={`bg-${service.color}-100 text-${service.color}-700 rounded-full p-1 mr-3`}>
                  ...
                  >
                    {openDropdown === service.id ? 'Hide details' : service.linkText}
                    <ChevronDown 
                      size={20} 
                      className={`ml-2 transition-transform duration-300 ${openDropdown === service.id ? 'rotate-180' : ''}`} 
                    />
                  </button>
                </div>
              </div>
              ...
              {openDropdown === service.id && (
                <div className="bg-gray-50 px-8 py-6 border-t border-gray-100 animate-slideDown">
                  <div className="mb-8">
                    <h3 className="text-xl font-bold text-gray-800 mb-4">Detailed Information</h3>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      {service.galleryImages.map((img, idx) => (
                        <div key={idx} className="rounded-lg overflow-hidden shadow">
                          <img src={img} alt={`${service.title} image ${idx + 1}`} className="w-full h-auto" />
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    <Link
                      to={service.link}
                      className={`text-white bg-${service.color}-600 hover:bg-${service.color}-700 py-3 px-6 rounded-lg transition-colors font-medium`}
                    >
                      Learn more
                    </Link>
                    ...
                    <a 
                      href="tel:+123456789" 
                      className="inline-flex items-center bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 py-3 px-6 rounded-lg transition-colors font-medium"
                    >
